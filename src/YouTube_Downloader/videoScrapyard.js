...
async function videoScrapyard(url) {
  return new Promise(async (resolve, reject) => {
    axios({
      ...
      var videoSize = vr("div")
        ...
        .text();
      var videoType = vr("div")
        .find("#mp4 > table > tbody > tr > td:nth-child(3) > a")
        .attr("data-ftype");
      var videoQuality = vr("div")
        .find("#mp4 > table > tbody > tr > td:nth-child(3) > a")
        .attr("data-fquality");
      ...
      await axios({
        ...
        data: {
          ...
          token: "",
          ftype: videoType,
          fquality: videoQuality,
        },
      }).then(function (body) {
        var vr = load(body.data.result);
        var videoLink = vr('div[class="form-group has-success has-feedback"]')
          .find("a")
          .attr("href");
        resolve({
          videoSize,
